  ...
  try {
    ...
    while (hasNextPage) {
      res = await apiCaller({
        shop,
        accessToken,
        endpoint: `customers.json?limit=100&page_info=${nextPageInfo}`,
        pageInfo: true,
      })
...
const findById = async ({ shop, accessToken, id }) => {
  try {
    ...
    return await apiCaller({ shop, accessToken, endpoint: `customers/${id}.json` })
  } catch (error) {
    throw error
  }
}
...
const create = async ({ shop, accessToken, data }) => {
  try {
    ...
    return await apiCaller({ shop, accessToken, endpoint: `customers.json`, method: 'POST', data })
  } catch (error) {
    throw error
  }
}
